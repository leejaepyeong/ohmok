...
public class TicTacToe : MonoBehaviour
{
    ...
    public Image fieldTexture;
	public Image circleTexture;
    public Image crossTexture;
    public Image youTexture;
    public Image winTexture;
    public Image loseTexture;
    ...
    private static float WINDOW_HEIGHT = 400.0f;
	...
	void DrawFieldAndMarks()
	{
		...
		Rect rect = new Rect(Screen.width / 2 - WINDOW_WIDTH * 0.5f,
							 ...
							 WINDOW_WIDTH,
							 WINDOW_HEIGHT);
		Graphics.DrawTexture(rect, fieldTexture.mainTexture);
		...
		for (int index = 0; index < spaces.Length; ++index)
		{
			if (spaces[index] != -1)
			{
				...
				Texture texture = (spaces[index] == 0) ? circleTexture.mainTexture : crossTexture.mainTexture;
		...
		if (localMark == turn)
		{
			...
			Graphics.DrawTexture(rect, youTexture.mainTexture);
		}
	}
	...
	void DrawWinner()
	{
		...
		{
			Graphics.DrawTexture(rect, winTexture.mainTexture);
		}

		if (localMark == Mark.Circle && winner == Winner.Cross ||
			localMark == Mark.Cross && winner == Winner.Circle)
		{
			Graphics.DrawTexture(rect, loseTexture.mainTexture);
		}
	}
